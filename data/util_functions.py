...
def subgraph_extraction_labeling(ind, Arow, Acol, h=1, sample_ratio=1.0, max_nodes_per_hop=None, 
    ...
    for dist in range(1, h+1):
        ...
        u_fringe -= u_visited
        v_fringe -= v_visited
        u_visited.update(u_fringe)
        v_visited.update(v_fringe)

        # Ensure u_fringe and v_fringe are lists before sampling
        u_fringe_list = sorted(u_fringe)  # Convert set to list
        v_fringe_list = sorted(v_fringe)  # Convert set to list

        # Apply sampling if sample_ratio < 1.0
        if sample_ratio < 1.0:
            sample_size = min(int(sample_ratio * len(u_fringe_list)), len(u_fringe_list))
            u_fringe_list = random.sample(u_fringe_list, sample_size) if u_fringe_list else []
            sample_size = min(int(sample_ratio * len(v_fringe_list)), len(v_fringe_list))
            v_fringe_list = random.sample(v_fringe_list, sample_size) if v_fringe_list else []

        # Apply max_nodes_per_hop constraint
        if max_nodes_per_hop is not None:
            if max_nodes_per_hop < len(u_fringe_list):
                u_fringe_list = random.sample(u_fringe_list, max_nodes_per_hop)
            if max_nodes_per_hop < len(v_fringe_list):
                v_fringe_list = random.sample(v_fringe_list, max_nodes_per_hop)

        if not u_fringe_list and not v_fringe_list:
            break

        u_nodes += u_fringe_list
        v_nodes += v_fringe_list
        u_dist += [dist] * len(u_fringe_list)
        v_dist += [dist] * len(v_fringe_list)
        u_fringe = set(u_fringe_list)  # Update fringe as set for next iteration
        v_fringe = set(v_fringe_list)  # Update fringe as set for next iteration

    subgraph = Arow[u_nodes][:, v_nodes]
    subgraph[0, 0] = 0  # Remove target link
    ...
    node_features = [u_features[0], v_features[0]] if (u_features is not None and v_features is not None) else None
